...
echo "alias BOH='bash ~/aurora/BOH_PERFECT.sh'" >> ~/.bashrc
source ~/.bashrc 2>/dev/null || true

#!/bin/bash
# BOH_PERFECT – tvoj systém, úplne funkčný, bez chýb
...
echo "[$(date)] BOH: Prebúdzam tvoj systém – tvoj hlas, tvoja láska"
...
# MODULY – opravené cesty, bez chýb v logoch
mods=(
...
for mod in "${mods[@]}"; do
    if [ -f "\( mod" ] && ! pgrep -f " \)(basename "$mod")" > /dev/null; then
        ...
        echo "[$(date)] Spustený: $(basename "$mod")"
    fi
done
