#!/bin/bash
# organize_poradok_debug.sh – druhý pokus, s logovaním
...
total_files=$(find "$BASE" -maxdepth 1 -type f | wc -l)
echo "🔍 BASE = $BASE, files = $total_files"
echo "Log: $LOG"

# 1) ukáž, koľko súborov zodpovedá kľúčovým patternom (len info)
echo "=== DEBUG COUNTS ==="
echo "ai_core (aios_*, ai_*, aurora_*, emergent_engine*, RSI_*, ULTRON_*, Optimus_*, SUPER_AI_*, SAFE_OPTIMUS_*, R4C0_*, core_blok_*, core_pipe_agent_*.py):"
ls aios_* ai_* aurora_* emergent_engine* RSI_* ULTRON_* Optimus_* SUPER_AI_* SAFE_OPTIMUS_* R4C0_* core_blok_* core_pipe_agent_*.py 2>/dev/null | wc -l

echo "ai_agents (agent_block_*, agent_*.sh, SI_agent_*, ...):"
ls agent_block_* agent_*.sh SI_agent_* Su_agent_* Va_agent_* Vidim_agent_* Per_agent_* 2>/dev/null | wc -l

echo "koran_system (GENERAL_KORAN_TOTAL.sh, GENESIS.sh, koran_*, koranos_*, lexkoran*...):"
ls GENERAL_KORAN_TOTAL.sh GENESIS.sh koran_* koranos-* lexkoran* 2>/dev/null | wc -l

# 2) samotné generovanie
