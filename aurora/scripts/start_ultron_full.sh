...
SERVER_LOG=~/aurora/logs/SERVER.log
...
echo "${CYAN}[INFO]$(date +'%F %T') | Reštartujem ULTRON MONOLITH server na porte 10010...${RESET}" | tee -a "$SERVER_LOG"

# === Spustenie servera ===
nohup python3 ~/ai_ultron_MONOLITH.py --port 10010 >> "$CENTRAL_LOG" 2>&1 &
SERVER_PID=$!
echo "${GREEN}[SERVER] Spustený PID=$SERVER_PID na porte 10010${RESET}" | tee -a "$SERVER_LOG"

# === Spustenie agentov (bez tohto skriptu samotného) ===
find ~/aurora/scripts -type f \
  \( -iname "*agent*.sh" -o -iname "*agent*.py" -o -iname "*ultron*.sh" -o -iname "*monolith*.sh" -o -iname "*core*.sh" \) \
  ! -name "start_ultron_full.sh" \
...
# === Centrálne live sledovanie: jadro + server + eventy ===
echo "${CYAN}[LIVE] Spúšťam centrálne sledovanie logov...${RESET}"
tail -f "$CENTRAL_LOG" "$SERVER_LOG" "$WATCHDOG_LOG"
