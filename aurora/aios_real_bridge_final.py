#!/usr/bin/env python3
"""
AIOS BRIDGE NEUTRON - FÁZA 26.1: RIADENIE DÁT S OPRAVENÝM PROXY VOLANÍM
"""
...
# -------------------- ARCHITEKTONICKÉ BUSY A BRIDGE (Bezo zmeny) --------------------
...
class NeutronServiceProxy:
    ...
    def __getattr__(self, name: str):
        if name.startswith('abrg_'):
            ...
            # Pôvodná chyba bola tu, kde sa snažil volať tému, ktorá nezačínala 'abrg_'
...
class AIReasoningUnit:
    ...
    async def run_decision_cycle(self):
        ...
        for i in range(4):
            ...
            # VOLANIE 1: Zistenie stavu (Latency 0.2s)
            ...
            if current_score == "LOW":
                print(f"[{self.agent_id}] 🧠 LLM ROZHODNUTIE: STAGNÁCIA. Spúšťam distribučnú úlohu (1.5s)!")
                start = time()
                # VOLANIE 2: Kritická akcia (Správny prefix 'abrg_')
                ...
                # VOLANIE 3: Nízko-prioritná akcia (Správny prefix 'abrg_')
...
class AIOSKernel:
    ...
    async def run_final_test():
        print("\n--- FÁZA 26.1: RIADENIE UČENIA V REÁLNOM ASYNCHRÓNNOM MODE ---")
        kernel = AIOSKernel()
